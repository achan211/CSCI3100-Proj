...
let Attendance = (props) => {
    ...
    useEffect(() => {
        if (props.type === 'prof') {
            if (props.course) {
                axios.get(`http://localhost:5000/attendance/teacher/getAttendance/${props.course}`, { withCredentials: true }).then(response => response.data).then((response) => {
            ...
            if (props.course) {
                console.log("sadkhs")
                axios.get(`http://localhost:5000/attendance/student/${props.course}`, { withCredentials: true }).then(response => response.data).then((response) => {
                    if (response.redirectURL) {
                        //back to login
                        window.location.href = 'http://localhost:3000' + response.redirectURL
                    }
