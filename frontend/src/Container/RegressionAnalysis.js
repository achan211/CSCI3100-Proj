...
let RegressionAnalysis = (props) => {
    ...
    useEffect(() => {
        ...
        if (Course && filered.length > 0) {
            ...
            axios.get(`http://localhost:5000/quiz/result/all/${Course}`, { withCredentials: true }).then(response => response.data).then((response) => {
                if (response.redirectURL) {
                    //back to login
                    window.location.href = 'http://localhost:3000' + response.redirectURL
                }
            ...
            axios.get(`http://localhost:5000/rating/result/${Course}`, { withCredentials: true }).then(response => response.data).then((response) => {
                if (response.redirectURL) {
                    //back to login
                    window.location.href = 'http://localhost:3000' + response.redirectURL
                }
    ...
    let renderAnalysis = () => {
        return (
            <React.Fragment>
                ...
                <FullPaperPageHeader width={'100%'}  title={`${props.match.params.course === undefined ? '' : Course} Regression Analysis`} 
